...
export default ({ bounds = { x: null, y: null } } = {}) => {
  ...
  const onTouchMove = ({ touches }) => {
    ...
    result.x = result.bounds.x ? clamp(dragValOrigin.x - diff.x, result.bounds.x[0], result.bounds.x[1]) : dragValOrigin.x - diff.x;
    result.y = result.bounds.y ? clamp(dragValOrigin.y + diff.y, result.bounds.y[0], result.bounds.y[1]) : dragValOrigin.y + diff.y;
  };
